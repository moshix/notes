...
void display_topics(MYSQL *conn, WINDOW *topics_win) {
    ...
    int total_width = title_width + user_width + notes_width + date_width + follow_by_width + status_width + 10; // 10 spaces between columns

    // Create a wide enough window to contain the title line
    if (total_width > COLS) {
        topics_win = newwin(LINES - 3, total_width, 3, 0);
    } else {
        topics_win = newwin(LINES - 3, COLS, 3, 0);
    }

    // Print the title line in a single wprintw call
    wattron(topics_win, A_REVERSE);
    wprintw(topics_win, "%-*s  %-*s  %-*s  %-*s  %-*s  %-*s\n", 
            title_width, "Title", 
            user_width, "User", 
            notes_width, "Notes", 
    ...
    if (mysql_query(conn, "SELECT title, created_by, feedback_count, DATE_FORMAT(last_update, '%Y-%m-%d %H:%i'), closed, last_follow_by FROM topics")) {
    ...
    while ((topic = mysql_fetch_row(result))) {
        ...
        strncpy(truncated_title, topic[0], title_width);
...
int main() {
    ...
    // Ensure the window is wide enough to contain the full title line
